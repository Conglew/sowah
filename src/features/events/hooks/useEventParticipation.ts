...
type UseEventParticipationResult = {
   ...
   *
   * 這是樂觀更新：按下去的當下就要看到結果，不能等一輪 refetch。
   * 伺服器重抓回來的資料才是真的，呼叫端應該用 onParticipationChange 觸發重抓，
   * 讓 participant_count 這種只有後端知道的欄位跟上。
   */
  ...
  /**
   * 拿伺服器的最新結果回來對帳：已經一致的 override 就丟掉，
   * 讓伺服器重新成為唯一真相。不一致的先留著——那代表後端還沒跟上，
   * 這時候把 override 清掉，畫面會倒退回使用者剛剛才改掉的狀態。
   */
  clearSettledOverrides: (
    serverEvents: { id: string; isJoinedByMe: boolean }[],
  ) => void;
};

type UseEventParticipationOptions = {
  /**
   * 報名／取消在後端成功後呼叫一次。用來讓呼叫端靜默重抓伺服器資料。
   * 行事曆的成敗不影響它——行事曆是衍生資料，跟後端的報名狀態無關。
   */
...
export function useEventParticipation(
  options: UseEventParticipationOptions = {},
): UseEventParticipationResult {
  const [pending, setPending] =
    useState<UseEventParticipationResult["pending"]>(null);

  // state 的 pending 是給畫面看的，這支 ref 才是真正的鎖。
  // setPending 要等下一次 render 才看得到，同一個 tick 內連點兩下會兩次都看到 null
  // 而雙雙放行；ref 是同步的，關得掉這個縫。
  const pendingRef = useRef(false);

  // 呼叫端通常傳行內箭頭函式，每次 render 都是新的一顆。直接放進下面兩個
  // useCallback 的 deps，joinEvent / cancelEvent 就會每次 render 都換掉，
  // 底下每一列 EventRow 跟著白重繪。用 ref 接住最新的那顆，deps 才穩得下來。
  const onParticipationChangeRef = useRef(options.onParticipationChange);

  useEffect(() => {
    onParticipationChangeRef.current = options.onParticipationChange;
  });
  ...
  const joinEvent = useCallback(async (event: ParticipatableEvent) => {
    ...
    setParticipationOverrides((previous) => ({
      ...previous,
      [event.id]: true,
    }));
    onParticipationChangeRef.current?.();
  ...
  const cancelEvent = useCallback(async (event: ParticipatableEvent) => {
    if (pendingRef.current) return;

    // 鎖在確認對話框之前就拿：等使用者回答再鎖，中間那段空窗可以疊開第二個對話框，
    // 兩個都按確定就會送出兩次取消。
    pendingRef.current = true;
  ...
  const clearSettledOverrides = useCallback(
    (serverEvents: { id: string; isJoinedByMe: boolean }[]) => {
      setParticipationOverrides((previous) => {
        let changed = false;
        const next = { ...previous };
        ...
        for (const event of serverEvents) {
          if (next[event.id] === event.isJoinedByMe) {
            delete next[event.id];
            changed = true;
          }
        }

        // 沒東西可清就回原本那顆，別製造一次無意義的 re-render
