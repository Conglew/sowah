...
> 這支原本規劃成 `DELETE /events/{eventId}/join`，實際串接時後端提供的是
> `POST .../leave`，已以實作為準。語意上 `leave` 也比 `DELETE join` 準確：
> 被刪掉的是參加關係，不是那張報名單。
...
前端另外不對**主辦人**顯示取消按鈕：主辦人要的是刪除活動（`DELETE /events/{eventId}`），
不是退出自己開的場。後端若收到主辦人的 leave，請明確回錯而不要靜默成功。
...
（`POST` 不是冪等方法，所以這件事只能靠上面這段伺服器端的判斷，不能靠 HTTP 語意。）
